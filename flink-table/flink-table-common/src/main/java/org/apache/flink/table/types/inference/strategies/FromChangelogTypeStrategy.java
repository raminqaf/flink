...
public final class FromChangelogTypeStrategy {
     ...
     * is not present in the {@code op_mapping}.
     *
     * <ul>
     *   <li>{@code FAIL} — throw an exception when an unmapped operation code is encountered
     *       (strict mode)
     *   <li>{@code LOG} — log a warning and drop the row
     *   <li>{@code SKIP} — silently drop the row (current default behavior)
     * </ul>
     */
    public enum InvalidOpHandlingMode {
        ...
        public static boolean isValid(final String mode) {
            return Arrays.stream(values()).anyMatch(v -> v.name().equalsIgnoreCase(mode.trim()));
        }
    ...
    private static Optional<List<DataType>> validateInvalidOpHandling(
        ...
        if (hasInvalidOpHandlingArgProvided && !isInvalidOpHandlingArgLiteral) {
            return callContext.fail(
                    throwOnFailure, "The 'invalid_op_handling' argument must be a constant STRING literal.");
        }
        ...
        if (optionalInvalidOpHandlingArg.isPresent()) {
            ...
            final String handlingStrategy = invalidOpHandlingMode.trim().toUpperCase();
            if (!InvalidOpHandlingMode.isValid(handlingStrategy)) {
                return callContext.fail(
                        throwOnFailure,
                        String.format(
                                "Invalid value for argument 'invalid_op_handling': '%s'. Valid values are: %s.",
                                handlingStrategy, InvalidOpHandlingMode.validValues()));
            }
        }
